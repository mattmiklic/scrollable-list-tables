...
function scrollable_list_tables_enqueue_assets() {
	...
	wp_enqueue_style( $handle, $url . 'list-tables.css', array( 'list-tables' ), '0.1.6' );
	wp_enqueue_script( $handle, $url . 'list-tables.js', array(), '0.1.6', true );
	wp_localize_script(
		$handle,
		'scrollableListTablesSettings',
		array(
			'label'    => $label ? $label : __( 'Items list', 'scrollable-list-tables' ),
			'previous' => __( 'Scroll to previous columns', 'scrollable-list-tables' ),
			'next'     => __( 'Scroll to next columns', 'scrollable-list-tables' ),
		)
	);
}
